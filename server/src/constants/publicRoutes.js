import { ROUTE_MOUNTS } from './routeMounts.js';
...
/**
 * Router name -> mount path, derived from the ordered runtime descriptors.
 *
 * `scim` is mounted like any other router so the guard sweep covers it, but it
 * appears in no public descriptor set below: its routes are service-authenticated
 * (§V60) and therefore neither public nor subject to the per-IP guest ceiling.
 */
export const ROUTER_MOUNTS = Object.freeze(Object.fromEntries(
  ROUTE_MOUNTS.map(({ name, path }) => [name, path]),
));
