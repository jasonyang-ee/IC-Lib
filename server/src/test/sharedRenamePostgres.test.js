...
describe('Shared rename lifecycle on scratch PostgreSQL', () => {
  ...
  beforeAll(async () => {
    ...
    await database.query(`
      ...
      CREATE TABLE components (
        id UUID PRIMARY KEY, part_number TEXT, approval_status TEXT, description TEXT,
        manufacturer_pn TEXT, value TEXT, package_size TEXT, manufacturer_id UUID, category_id UUID,
        approval_user_id UUID, approval_date TIMESTAMP, updated_at TIMESTAMP,
        pad_file TEXT, pcb_footprint TEXT, schematic TEXT, step_model TEXT, pspice TEXT
      );
    ...
    for (const table of ['cad_files', 'component_cad_files', 'eco_approval_stages', 'eco_settings', 'eco_orders', 'eco_changes', 'eco_approvals', 'eco_stage_approvers', 'eco_file_rename_files', 'eco_file_rename_components']) {
  ...
  beforeEach(async () => {
    ...
    await database.query(`
      ...
      TRUNCATE users, components, cad_files, component_cad_files, eco_settings, eco_orders,
        ...
        eco_file_rename_components, activity_log;
  ...
  it.each([['single', renamePhysicalFile], ['group', renameFootprintGroup]])('refreshes %s rename consumers and exposes accurate staged counts', async (_, handler) => {
    ...
    poolProxy.query.mockImplementation(async (sql, ...args) => {
      ...
      if (!changed && sql.includes('c.approval_status') && sql.includes('WHERE ccf.cad_file_id')) {
        changed = true;
