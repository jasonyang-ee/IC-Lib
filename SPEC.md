...
U.roles: 6 roles `read-only|reviewer|lab|read-write|approver|admin`. `/login` = normal sign-in + guest read-only sign-in; success -> requested route if any.
...
- browse: category sidebar, full-text search, sort PN|MFG P/N|value|desc|created|updated, multi-status filter, prev/next step when search parses as PN, virtualized list, copy deep link.
...
U.vendor `/vendor-search`: find distributor data, seed part, append rows, add alts. text search PN|MFG P/N, barcode decode (Digikey Data Matrix + Mouser Code 128), camera scan, side-by-side Digikey+Mouser panes, multi-select cross-vendor, sessionStorage keeps results+selection. Actions: `Add to Library` -> prefilled draft (one source primary, all selections -> distributor rows); `Append to Existing Parts` -> exact MFG P/N match -> target primary|alt; no match -> modal target other part as new alt or fall back to new-part; download footprints Ultra Librarian|SnapEDA.
...
U.inv `/inventory`: locate stock, adjust qty/min, scan labels, copy QR, pivot Library|Vendor Search. filters category|project|approval|location; full-text across part + alt fields; sort PN|MFG P/N|qty|location|min|last-edited; vendor barcode + camera scan; low-stock card; label-template download. table: expand -> alts; read mode location/qty/min/Library jump/label; edit-all -> bulk location, direct qty set, consume, receive, min updates (primary + alt); QR/label modal + copy; barcode miss -> Vendor Search.
...
U.user `/user-settings`: account summary username/role; profile display name/email/file storage path; ECO notification prefs; delegated backup approver; password change.
