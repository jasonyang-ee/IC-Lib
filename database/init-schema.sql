...
-- Canonical package catalog. Built-in package and alias rows are seeded by
-- init-settings.sql so a later startup can safely re-run the seed.
...
-- Table: footprint_related_cad_files
-- Tracks reusable footprint-driven pad and 3D model links
...
CREATE INDEX IF NOT EXISTS idx_footprint_related_cad_files_footprint ON footprint_related_cad_files(footprint_cad_file_id);
CREATE INDEX IF NOT EXISTS idx_footprint_related_cad_files_related ON footprint_related_cad_files(related_cad_file_id);
CREATE INDEX IF NOT EXISTS idx_footprint_related_cad_files_related_type ON footprint_related_cad_files(related_file_type);
...
-- Table: eco_file_rename_files
-- Stores staged shared-file rename operations pending ECO approval.
...
-- Table: eco_file_rename_components
-- Stores the affected parts and their pre-review lifecycle status for staged shared-file renames.
