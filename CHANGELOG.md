...
- Database restores now preflight live schema metadata before deleting data, omit generated columns, preserve explicit identity values, batch inserts within PostgreSQL bind limits, keep foreign-key triggers active, and transactionally advance owned sequences. Failed imports roll back rows and sequence state.
...
- The PostgreSQL schema regression now uses either a dedicated `OIDC_SCHEMA_TEST_DATABASE_URL` with an isolated disposable schema or an owned local PostgreSQL 18 data directory. It proves server version before any schema write; local runs also verify the live data directory, `postmaster.pid` port, and child process, so an unrelated PostgreSQL listener cannot receive test DDL. CI supplies the PostgreSQL 18 service URL explicitly, and regressions reject PostgreSQL 16 or a mismatched occupied port before writes.

- Public-route resolution and SCIM parser bypass now mirror Express case-insensitive routing and implicit HEAD handling, so spelling or method variants cannot escape the guest limiter or parse an unauthenticated SCIM body. The bundled image binds Node only to `127.0.0.1`, nginx proxies to that loopback address, and direct deployments must choose an explicit IP bind with `TRUST_PROXY_HOPS=0` unless a real trusted proxy exists. Route guards now reject path-scoped authentication as a router-wide gate and reject unregistered direct API mounts apart from the named health probes.

- ECO alternative-class staging now requires both `old_value` and `new_value` before opening a database connection, then validates each value and returns the stable 400 domain error for omissions or invalid input. The Library's bulk class modal locks selection, Apply, Cancel, and close controls while its one request is pending, retains an error for retry, and snapshots only currently visible, eligible component IDs so filtering cannot redirect a stale selection to hidden rows.

- Library bulk deletion now sends one `DELETE /api/components/bulk` request instead of parallel per-row deletes. The server validates a bounded, unique UUID set, locks every target before applying the ECO direct-edit policy, then deletes dependencies, components, and required audit rows in one transaction; any missing, controlled, or failed member rolls back the whole operation. The confirmation stays visible and retryable until the request succeeds.

- SCIM now canonicalizes tenant/object/local UUID boundaries to lowercase, refuses sensitive dotted/filtered attribute paths before any database query, tolerates Entra's empty `roles` array while ignoring POST `id`/`meta`, and exposes stable 201 `Location` responses plus retrievable User ResourceType/Schema metadata. `externalId` is advertised as server-unique/immutable.

- SCIM attribute names now normalize case-insensitively throughout create and PATCH payloads, while duplicate case variants reject safely instead of selecting an arbitrary value. Authenticated parser and unexpected controller failures retain the SCIM error media/schema, and every user write rechecks the selected tenant/object link so a concurrent ownership change returns a conflict without recording a lifecycle success.
...
- Migration `21_package_catalog.sql` introduces the PostgreSQL 18 package catalog foundation: canonical packages, separator-insensitive generated alias keys, validated count policies, and idempotent fresh-schema and upgrade paths.

- The package catalog’s startup seed now supplies 116 built-in packages and their normalized aliases, including each canonical self-alias. Repeated startup seeds preserve soft-disabled built-ins and site-added packages without changing catalog counts.

- Startup schema inspection and database backup/export coverage now include the package catalog tables, so a missing catalog table blocks readiness and catalog edits are preserved in exports.
