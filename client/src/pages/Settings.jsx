...
const ApprovalStagesSettings = () => {
  ...
  const [newStage, setNewStage] = useState({ stage_name: '', required_approvals: 1, required_role: 'approver' });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setIsAdding(false);
      setNewStage({ stage_name: '', required_approvals: 1, required_role: 'approver' });
  ...
  const reorderMutation = useMutation({
    mutationFn: (stageIds) => api.reorderApprovalStages(stageIds),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['approvalStages'] });
    },
    onError: (error) => {
      ...
      {isAdding && (
        <div className="mb-4 p-4 bg-gray-50 dark:bg-[#333] rounded-lg border border-gray-200 dark:border-[#444]">
          ...
          <div className="flex gap-2 mt-3">
            ...
            <button
              onClick={() => { setIsAdding(false); setNewStage({ stage_name: '', required_approvals: 1, required_role: 'approver' }); }}
