...
# Read current version from root package.json or client/package.json
get_current_version() {
    if [ -f package.json ]; then
        node -p "require('./package.json').version" 2>/dev/null || echo "1.0.0"
    elif [ -f client/package.json ]; then
        node -p "require('./client/package.json').version" 2>/dev/null || echo "1.0.0"
    else
        echo "1.0.0"
    fi
}
